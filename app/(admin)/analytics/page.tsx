  ...
  return (
    <div className="container py-8 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Platform-wide metrics and conversion funnels.</p>
      </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ${(totalRequested / 100).toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Capacity Remaining</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              ${((totalCapacity - totalUsed) / 100).toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </div>
            <p className="text-xs text-muted-foreground mt-1">
              of ${(totalCapacity / 100).toLocaleString()} limit
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {capacityData?.filter(s => s.status === 'active').length ?? 0}
            </div>
          </CardContent>
        </Card>
      </div>
                  ...
                  <div key={sponsor.id}>
                    ...
                    <div className="w-full bg-muted rounded-full h-2.5">
                      <div
                        ...
                        style={{ width: Math.min(sponsor.utilization_pct, 100) + '%' }}
                      />
                    </div>
                  </div>
                ))}
                ...
                .map(pitch => (
                  <div key={pitch.id} className="flex justify-between items-center border-b pb-3 last:border-0 last:pb-0">
                    ...
                    <div className="text-right">
                      <span className="inline-block px-2 py-1 bg-muted rounded text-xs capitalize">
