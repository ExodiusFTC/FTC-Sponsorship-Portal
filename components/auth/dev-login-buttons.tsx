...
export function DevLoginButtons() {
  ...
  const [active, setActive] = useState<'coach' | 'admin' | 'sponsor' | null>(null)

  function login(role: 'coach' | 'admin' | 'sponsor') {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      ...
      <button
        onClick={() => login('sponsor')}
        disabled={isPending}
        style={{
          padding: '12px', borderRadius: '8px', fontSize: '14px', fontWeight: 500,
          cursor: isPending ? 'not-allowed' : 'pointer', border: '1px solid #374151',
          background: active === 'sponsor' && isPending ? '#111' : '#18181b',
          color: '#d4d4d8', transition: 'background 0.15s',
        }}
      >
        {isPending && active === 'sponsor' ? 'Signing in…' : '🤝 Login as Test Sponsor'}
      </button>
      <button
        onClick={() => login('admin')}
        disabled={isPending}
